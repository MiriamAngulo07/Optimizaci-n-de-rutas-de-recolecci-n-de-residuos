...
# ─── ① CARGAR TU GRAFO ────────────────────────────────────────────────────────
# OPCIÓN A: Si tienes el grafo guardado en GraphML (recomendado)
#   G = ox.load_graphml("mi_grafo_don_juan.graphml")
#
# OPCIÓN B: Si tienes el grafo en memoria en otro script,
#   copia este código al final de ese script y pasa G directamente.
#
# OPCIÓN C (demo): Construir grafo sintético con nodos ficticios
#   (para probar el CVRP sin conexión a internet)

print("🔧 Construyendo grafo sintético para demostración...")
print("   (Reemplaza esta sección con tu G de OSMnx)")

# Grafo sintético con coordenadas reales de Puerto Caimito
# Simula una cuadrícula de calles en la zona de Don Juan
G = nx.MultiDiGraph()
G.graph["crs"] = "epsg:4326"

lat_base, lon_base = 8.880, -79.762
step_lat = 0.002   # ~220 metros
step_lon = 0.002
...
for i in range(filas):
    for j in range(cols):
        lat = lat_base + i * step_lat
        lon = lon_base + j * step_lon
        G.add_node(nodo_id, y=lat, x=lon, street_count=4)
        grid[(i, j)] = nodo_id
        nodo_id += 1
...
for i in range(filas):
    for j in range(cols):
        dist_h = 220   # metros aprox
        dist_v = 220
        if j + 1 < cols:
            u, v = grid[(i, j)], grid[(i, j+1)]
            G.add_edge(u, v, length=dist_h, speed_kph=30, travel_time=dist_h/30*3.6)
            G.add_edge(v, u, length=dist_h, speed_kph=30, travel_time=dist_h/30*3.6)
        if i + 1 < filas:
            u, v = grid[(i, j)], grid[(i+1, j)]
            G.add_edge(u, v, length=dist_v, speed_kph=30, travel_time=dist_v/30*3.6)
            G.add_edge(v, u, length=dist_v, speed_kph=30, travel_time=dist_v/30*3.6)
...
print(f"\n📍 Seleccionando {NUM_PUNTOS} puntos de recolección...")
...
demandas = [0] + [random.randint(40, 120) for _ in nodos_recoleccion]
